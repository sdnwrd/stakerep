...
function CrashGame({ balance, updateBalance, addToHistory }) {
  ...
  const startGame = () => {
    ...
    const animate = () => {
      ...
      // Independent speed calculation - starts slow, accelerates exponentially
      // Speed is based on current multiplier, not crash point
      const baseSpeed = 0.001; // Very slow start
      const acceleration = Math.pow(currentMultiplier, 1.8); // Exponential acceleration
      const currentSpeed = baseSpeed * acceleration;

      const newMultiplier = currentMultiplier + currentSpeed;
